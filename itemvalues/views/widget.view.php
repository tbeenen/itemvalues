...
use Widgets\ItemValues\ItemValuesWidget as Widget;
	...
	foreach ($classes_vertical as $row_key => $row_class) {
		...
		foreach ($classes_horizontal as $column_key => $column_class) {
			...
			switch ($cell_type) {
				...
				case 'item_value':
					// ** Nouvelle logique pour gérer les retours à la ligne dans la valeur **
					if (array_key_exists('value', $cell_data['parts'])) {
						$value_text = $cell_data['parts']['value']['text'];
						
						if (is_string($value_text) && strpos($value_text, "\n") !== false) {
							// Utiliser la même fonction Zabbix que pour la description
							$cell_data['parts']['value']['text'] = zbx_nl2br($value_text);
							$div->addClass('multiline'); // Ajouter une classe CSS si nécessaire
						}
					}
					$div
						->addClass('item-value')
